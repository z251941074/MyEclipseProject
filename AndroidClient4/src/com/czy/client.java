	...
	public void Conn() {
		if(isStop){
			...
			new Thread() {
				...
				public void run() {
	            	try {
						URL url =new URL(conStr);  // 开启socket http请求，客户端Socket
						
						Socket server = new Socket(url.getHost(), url.getPort());  //建立输入输出流
						...
						request.append("\r\n");
						os.write(request.toString().getBytes(), 0, request.length());  //向输出流写入数据

						StreamSplit localStreamSplit = new StreamSplit(new DataInputStream(new BufferedInputStream(is)));   //用于处理图像数据流
						...
						do{
							...
							if (str3.startsWith("multipart/x-mixed-replace")){
								...
								Message message = Message.obtain();    //Android更推荐我们通过Message.obtain()或者Handler.obtainMessage()获取Message对象
								message.arg1 = 0;
